...
#Region "Frm Code"
    ...
    Private Sub frmEditor_Events_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For i = 1 To MAX_SKILLS
            cmbChangeSkills.Items.Add(Skill(i).Name)
        Next
        ...
        For i = 1 To MAX_MAP_NPCS
            If MyMap.NPC(i) > 0 Then
                cmbSpawnNpc.Items.Add(i & ". " & Type.NPC(MyMap.NPC(i)).Name)
            Else
...
#Region "Graphic"
    ...
    Private Sub CmbGraphic_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbGraphic.SelectedIndexChanged
        ...
        If TmpEvent.Pages(CurPageNum).GraphicType = 1 Then
            If Me.nudGraphic.Value <= 0 Or Me.nudGraphic.Value > GameState.NumCharacters Then Exit Sub
    ...
    Private Sub PicGraphicSel_Click(sender As Object, e As MouseEventArgs)
        ...
        Dim selW As Integer = Math.Ceiling(X \ GameState.PicX) - GraphicSelX
        Dim selH As Integer = Math.Ceiling(Y \ GameState.PicY) - GraphicSelY
        ...
        If cmbGraphic.SelectedIndex = 2 Then
            If Control.ModifierKeys = Keys.Shift Then
                ...
                End If
            Else
                GraphicSelX = Math.Ceiling(X \ GameState.PicX)
                GraphicSelY = Math.Ceiling(Y \ GameState.PicY)
                GraphicSelX2 = 1
                GraphicSelY2 = 1
            End If
        ElseIf cmbGraphic.SelectedIndex = 1 Then
            ...
            GraphicSelX2 = 0
            GraphicSelY2 = 0
            If nudGraphic.Value <= 0 Or nudGraphic.Value > GameState.NumCharacters Then Exit Sub
            For i = 0 To 3
                If GraphicSelX >= GameClient.GetGfxInfo(IO.Path.Combine(Core.Path.Characters, nudGraphic.Value)).Width / 4 * i And GraphicSelX < GameClient.GetGfxInfo(IO.Path.Combine(Core.Path.Characters, nudGraphic.Value)).Width / 4 * (i + 1) Then
                    GraphicSelX = i
                End If
            Next
            For i = 0 To 3
                If GraphicSelY >= GameClient.GetGfxInfo(IO.Path.Combine(Core.Path.Characters, nudGraphic.Value)).Height / 4 * i And GraphicSelY < GameClient.GetGfxInfo(IO.Path.Combine(Core.Path.Characters, nudGraphic.Value)).Height / 4 * (i + 1) Then
                    GraphicSelY = i
                End If
            Next
        End If
