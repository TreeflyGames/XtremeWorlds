...
#Region "Drawing"
    ...
    Friend Sub DrawPet(index As Integer)
        ...
        rect = New Microsoft.Xna.Framework.Rectangle(
            anim * (Client.CharacterGfxInfo(spriteNum).Width \ 4), 
            spriteleft * (Client.CharacterGfxInfo(spriteNum).Height \ 4),
            Client.CharacterGfxInfo(spriteNum).Width \ 4,
            Client.CharacterGfxInfo(spriteNum).Height \ 4
        )

        ' Calculate the X
        x = Type.Player(index).Pet.X * PicX + Type.Player(index).Pet.XOffset - ((Client.CharacterGfxInfo(spriteNum).Width / 4 - 32) / 2)

        ' Is the player's height more than 32..?
        If (Client.CharacterGfxInfo(spriteNum).Height / 4) > 32 Then
            ' Create a 32 pixel offset for larger sprites
            y = Type.Player(index).Pet.Y * PicY + Type.Player(index).Pet.YOffset - ((Client.CharacterGfxInfo(spriteNum).Width / 4) - 32)
        Else
            ' Proceed as normal
    ...
    Friend Sub DrawPlayerPetName(index As Integer)
        ...
        Else
            ...
            textY = ConvertMapY(Type.Player(index).Pet.Y * PicY) + Type.Player(index).Pet.YOffset - (Client.CharacterGfxInfo(Type.Pet(Type.Player(index).Pet.Num).Sprite).Height / 4) + 16
        End If
